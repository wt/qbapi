...
use crate::util::auth::{
    QBAuthData, get_qb_auth_data, get_stored_profile_auth_token, store_access_key,
};
...
enum SubCommands {
    /// Login to acquire an access token.
    Login(LoginArgs),
}
...
pub(crate) struct LoginArgs {
    ...
    ///
    /// This will force a new login with no refresh of an existing token.
    #[arg(long)]
    disable_token_refresh: bool,

    /// Hostname to listen for auth redirect response.
    #[arg(long, default_value = "localhost")]
    listen_host: String,

    /// Port to listen for auth reddirect response.
...
pub(crate) async fn do_login(login_args: &LoginArgs, project_dirs: &ProjectDirs) -> Result<()> {
    ...
    let api_client = login_args.environment.api_client();
